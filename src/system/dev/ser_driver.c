...
#define guaranteed_delivery_streams_size                                                           \
    (sizeof(guaranteed_delivery_streams) / sizeof(*guaranteed_delivery_streams))

// global runtime config for the serial driver
static enum {
    E_COBS_ENABLED = 1
} ser_driver_runtime_config;
...
int ser_write_r(struct _reent* r, void* const arg, const uint8_t* buf, const size_t len) {
    ...
    if (!list_contains(
            guaranteed_delivery_streams,
            guaranteed_delivery_streams_size,
            file.stream_id
        )
...
void ser_driver_initialize(void) {
    ...
    write_stream =
        stream_buf_create_static(VEX_SERIAL_BUFFER_SIZE, 0, write_buf, &write_stream_buf);
