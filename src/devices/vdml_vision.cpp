...
vision_signature_s_t Vision::signature_from_utility(
    ...
    return c::vision_signature_from_utility(
        id,
        u_min,
        u_max,
        u_mean,
...
vision_color_code_t Vision::create_color_code(
    const std::uint32_t sig_id1,
    const std::uint32_t sig_id2,
    const std::uint32_t sig_id3,
    const std::uint32_t sig_id4,
    const std::uint32_t sig_id5
) const {
...
vision_object_s_t
Vision::get_by_sig(const std::uint32_t size_id, const std::uint32_t sig_id) const {
    return vision_get_by_sig(_port, size_id, sig_id);
}

vision_object_s_t
Vision::get_by_code(const std::uint32_t size_id, const vision_color_code_t color_code) const {
...
std::int32_t Vision::set_signature(
    const std::uint8_t signature_id,
