...
double imu_get_heading(uint8_t port) {
    ...
    double rtn = vexDeviceImuDegreesGet(device->device_info)
                 + ((imu_data_s_t*)registry_get_device(port - 1)->pad)->heading_offset;
...
int32_t imu_set_heading(uint8_t port, double target) {
    ...
    vexDeviceImuAttitudeGet(device->device_info, (V5_DeviceImuAttitude*)&euler_values);
    imu_data_s_t* data = (imu_data_s_t*)device->pad;
    if (target > IMU_HEADING_MAX)
        target = IMU_HEADING_MAX;
    if (target < 0)
        target = 0;
...
int32_t imu_set_pitch(uint8_t port, double target) {
    ...
    vexDeviceImuAttitudeGet(device->device_info, (V5_DeviceImuAttitude*)&euler_values);
    imu_data_s_t* data = (imu_data_s_t*)device->pad;
    if (target > IMU_EULER_LIMIT)
        target = IMU_EULER_LIMIT;
    if (target < -IMU_EULER_LIMIT)
        target = -IMU_EULER_LIMIT;
...
int32_t imu_set_euler(uint8_t port, euler_s_t target) {
    ...
    vexDeviceImuAttitudeGet(device->device_info, (V5_DeviceImuAttitude*)&euler_values);
    imu_data_s_t* data = (imu_data_s_t*)device->pad;
    if (target.pitch > IMU_EULER_LIMIT)
        target.pitch = IMU_EULER_LIMIT;
    if (target.pitch < -IMU_EULER_LIMIT)
        target.pitch = -IMU_EULER_LIMIT;
    if (target.yaw > IMU_EULER_LIMIT)
        target.yaw = IMU_EULER_LIMIT;
    if (target.yaw < -IMU_EULER_LIMIT)
        target.yaw = -IMU_EULER_LIMIT;
    if (target.roll > IMU_EULER_LIMIT)
        target.roll = IMU_EULER_LIMIT;
    if (target.roll < -IMU_EULER_LIMIT)
        target.roll = -IMU_EULER_LIMIT;
