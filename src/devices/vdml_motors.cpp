...
Motor::Motor(
    ...
    : Device(std::abs(port), DeviceType::motor),
      _port(port) {
...
std::int32_t
Motor::set_brake_mode(const pros::motor_brake_mode_e_t mode, const std::uint8_t index) const {
    if (index != 0) {
        errno = EOVERFLOW;
        return PROS_ERR;
    }
...
std::int32_t
Motor::set_brake_mode(const pros::v5::MotorBrake mode, const std::uint8_t index) const {
    if (index != 0) {
        errno = EOVERFLOW;
        return PROS_ERR;
    }
...
std::int32_t Motor::set_encoder_units(
...
std::int32_t
Motor::set_encoder_units(const pros::v5::MotorUnits units, const std::uint8_t index) const {
    if (index != 0) {
        errno = EOVERFLOW;
        return PROS_ERR;
    }
