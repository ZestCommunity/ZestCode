...
std::optional<DeviceInfo> set_device(uint8_t port, DeviceType type) {
	...
	// invalid port
	if (!is_valid_port(port)) return std::nullopt;
	// in use
	if (device_registry[port]->type != DeviceType::NONE) return std::nullopt;
	// device type mismatch
	if ((DeviceType)registry_types[port] != type && (DeviceType)registry_types[port] != DeviceType::NONE)
		return std::nullopt;
...
bool validate_device(uint8_t port, DeviceType expected) {
	...
	if ((expected == registered || expected == zest::vdml::DeviceType::NONE) && registered == actual) {
		// all good
		return true;
