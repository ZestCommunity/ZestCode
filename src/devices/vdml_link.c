...
static uint32_t _clear_rx_buf(v5_smart_device_s_t* device) {
    ...
    return vexDeviceGenericRadioReceive(
        device->device_info,
        (uint8_t*)buf,
...
// custom claim_port style wrapper for link_init due to type mismatching otherwise, limit of one
// radio per brain
static uint32_t _link_init(uint8_t port, const char* link_id, link_type_e_t type, bool ov) {
    ...
    if (plugged_type == E_DEVICE_RADIO) {
        ...
        // Hack: Force v5 to recognize the plugged type as a serial device the next time we touch
        // the device
...
uint32_t link_receive(uint8_t port, void* dest, uint16_t data_size) {
    ...
    if (START_BYTE != header_byte || received_size != 1) {
        kprintf(
            "[VEXLINK] Invalid Header Byte Received Port %d, header byte: %x",
            port,
            header_byte
        );
        errno = EBADMSG;
    ...
    received_size =
        vexDeviceGenericRadioReceive(device->device_info, (uint8_t*)&received_data_size, 2);
    if (received_size != 2 || received_data_size != data_size) {
        _clear_rx_buf(device);
        kprintf(
            "[VEXLINK] Invalid Data Size (Size: %d ) Received Port %d, flushing RX buffer!\n",
            received_data_size,
            port
        );
        errno = EBADMSG;
