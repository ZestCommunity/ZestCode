...
vision_object_s_t _vision_get_by_sig(uint8_t port, const uint32_t size_id, const uint32_t sig_id) {
    ...
    for (uint8_t i = 0; i <= object_count; i++) {
        ...
        if (vexDeviceVisionObjectGet(device->device_info, i, (V5_DeviceVisionObject*)&check)
            == PROS_ERR) {
            errno = EAGAIN;
            rtn = check;
            goto err_return;
...
vision_object_s_t
vision_get_by_code(uint8_t port, const uint32_t size_id, const vision_color_code_t color_code) {
    return _vision_get_by_sig(port, size_id, color_code);
}

int32_t vision_read_by_size(
    uint8_t port,
...
int32_t _vision_read_by_sig(
    ...
    for (uint8_t i = 0; i < c; i++) { // loop through all objects on sensor
        ...
        if (!vexDeviceVisionObjectGet(
                device->device_info,
                i,
                (V5_DeviceVisionObject*)(object_arr + j)
            )) {
            errno = EAGAIN;
            object_arr[i].signature = VISION_OBJECT_ERR_SIG;
            goto rtn;
    ...
    errno = EDOM; // read through all objects and couldn't find enough objects matching filter
                  // parameters
rtn:
    return_port(port - 1, j);
}

int32_t vision_read_by_sig(
    uint8_t port,
    const uint32_t size_id,
    const uint32_t sig_id,
    const uint32_t object_count,
...
vision_signature_s_t vision_get_signature(uint8_t port, const uint8_t signature_id) {
    ...
    rtn = vexDeviceVisionSignatureGet(
        device->device_info,
        signature_id,
        (V5_DeviceVisionSignature*)&sig
    );
    if (!rtn || !sig._pad[0]) { // sig._pad[0] is flags, will be set to 1 if data is valid and
                                // signatures are sent
        errno = EAGAIN;
        sig.id = VISION_OBJECT_ERR_SIG;
    }
...
vision_color_code_t vision_create_color_code(
    ...
    uint16_t id = 0;
    if (!sig_id1 || !sig_id2 || sig_id1 > 7 || sig_id2 > 7 || sig_id3 > 7 || sig_id4 > 7
        || sig_id5 > 7) {
...
int32_t vision_print_signature(const vision_signature_s_t sig) {
    ...
    printf(
        "%d, {%d, %d, %d}, %f, %ld, %ld, %ld, %ld, %ld, %ld, %ld, %ld};\n\n",
        sig.id,
        sig._pad[0],
        sig._pad[1],
        sig._pad[2],
