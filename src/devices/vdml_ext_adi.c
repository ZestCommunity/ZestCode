...
// These 2 functions aren't in v5_api.h but should be... so we're going to directly expose them with
// an extern "C".
#ifdef __cplusplus
extern "C" {
#endif
// private addressable LED API
int32_t vexDeviceAdiAddrLedSet(
    V5_DeviceT device,
    uint32_t port,
    uint32_t* pData,
    uint32_t nOffset,
...
#define validate_type(device, adi_port, smart_port, type)                                          \
    adi_port_config_e_t config =                                                                   \
        (adi_port_config_e_t)vexDeviceAdiPortConfigGet(device->device_info, adi_port);             \
    if (config != type) {                                                                          \
        errno = EADDRINUSE;                                                                        \
...
int32_t ext_adi_analog_calibrate(uint8_t smart_port, uint8_t adi_port) {
    ...
    for (uint32_t i = 0; i < 512; i++) {
        ...
        task_delay(1); // TODO: If smart ports (and the ADI) only update every 10ms, this really
                       // only reads 56 samples, maybe change to a 10ms
    }
...
int32_t ext_adi_analog_read_calibrated(uint8_t smart_port, uint8_t adi_port) {
    ...
    adi_data_s_t* const adi_data = &((adi_data_s_t*)(device->pad))[adi_port];
    int32_t rtn =
        vexDeviceAdiValueGet(device->device_info, adi_port) - (adi_data->analog_data.calib >> 4);
    return_port(smart_port - 1, rtn);
}
...
int32_t ext_adi_analog_read_calibrated_HR(uint8_t smart_port, uint8_t adi_port) {
    ...
    adi_data_s_t* const adi_data = &((adi_data_s_t*)(device->pad))[adi_port];
    int32_t rtn =
        (vexDeviceAdiValueGet(device->device_info, adi_port) << 4) - adi_data->analog_data.calib;
    return_port(smart_port - 1, rtn);
}
...
ext_adi_encoder_t ext_adi_encoder_init(
    uint8_t smart_port,
    uint8_t adi_port_top,
    uint8_t adi_port_bottom,
    bool reverse
) {
...
ext_adi_ultrasonic_init(uint8_t smart_port, uint8_t adi_port_ping, uint8_t adi_port_echo) {
...
ext_adi_led_t ext_adi_led_init(uint8_t smart_port, uint8_t adi_port) {
    ...
    vexDeviceAdiPortConfigSet(
        device->device_info,
        adi_port,
        (V5_AdiPortConfiguration)E_ADI_DIGITAL_OUT
    );
    return_port(smart_port - 1, merge_adi_ports(smart_port, adi_port + 1));
}
...
int32_t ext_adi_led_set(ext_adi_led_t led, uint32_t* buffer, uint32_t buffer_length) {
    ...
    uint32_t rtv = (uint32_t
    )vexDeviceAdiAddrLedSet(device->device_info, adi_port, buffer, 0, buffer_length, 0);
    return_port(smart_port - 1, rtv);
}

int32_t ext_adi_led_set_pixel(
    ext_adi_led_t led,
    uint32_t* buffer,
...
int32_t ext_adi_led_clear_pixel(
    ext_adi_led_t led,
    uint32_t* buffer,
    uint32_t buffer_length,
    uint32_t pixel_position
